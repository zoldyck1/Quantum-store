...
const ProductDetail = () => {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState('cmi');
  ...
  const getProductName = (productId) => {
    ...
    return productMap[productId] || 'Premium Digital Marketing Course';
  };
  ...
  const [cihAccountNumber, setCihAccountNumber] = useState('');
  const [cryptoPaymentLink, setCryptoPaymentLink] = useState('');
  ...
  const handlePaymentMethodChange = (methodId) => {
    ...
    setBillingInfo({ billingName: '', email: '', phone: '' });
    setCihAccountNumber('');
    setCryptoPaymentLink('');
  };
  ...
  const generateCryptoPaymentLink = () => {
    const amount = totalPrice;
    const orderId = `ORDER_${Date.now()}`;
    // This would typically call your crypto payment service
    const mockLink = `https://crypto-pay.example.com/pay?amount=${amount}&currency=USDT&orderId=${orderId}`;
    setCryptoPaymentLink(mockLink);
    toast.success('Lien de paiement crypto généré!');
  };
  ...
  const handleCMIPayment = async () => {
    if (!billingInfo.billingName || !billingInfo.email) {
    ...
    try {
      ...
      if (result.type === 'payment/processCMIPayment/fulfilled') {
        toast.success('Redirection vers CMI...');
      }
    } catch (error) {
      toast.error('Erreur lors du paiement CMI');
    }
  };

  const handleCIHPayment = () => {
    if (!cihAccountNumber) {
      toast.error('Veuillez entrer le numéro de compte CIH');
      return;
    }
    // Process CIH payment logic here
    toast.success(`Paiement CIH initié pour le compte ${cihAccountNumber}`);
  };
          ...
          <div className="grid grid-cols-4 gap-2">
            {product.images.map((image, index) => (
              <button
                ...
                className={`aspect-w-16 aspect-h-9 rounded-md overflow-hidden border-2 transition-colors ${
                  mainImage === image ? 'border-blue-400' : 'border-gray-600 hover:border-gray-500'
                }`}
              >
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <button 
                ...
                className={`p-2 rounded-full transition-colors ${
                  isFavorited ? 'text-red-400 bg-gray-700' : 'text-gray-400 hover:text-red-400 hover:bg-gray-700'
                }`}
              >
          ...
          <div className="bg-gray-800 p-6 rounded-lg">
            ...
            <div className="grid grid-cols-3 gap-2">
              {Object.entries(product.plans).map(([planKey, plan]) => (
                <div
                  key={planKey}
                  className={`border-2 rounded-md p-3 text-center cursor-pointer transition-all ${
